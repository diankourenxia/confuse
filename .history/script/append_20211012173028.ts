...
export function addXmlWithActivity(xmlFilePath:string,activityFilePath:string,packageInfo:string){
    let name = getUUID()
    let randomElementLength =  Math.floor((Math.random()*20)+1)
       //3. fs.writeFile  写入文件（会覆盖之前的内容）（文件不存在就创建）  utf8参数可以省略  
window.fs.writeFile(xmlFilePath+'/R'+name+'R.xml',`<?xml version="1.0" encoding="utf-8"?>
<layout xmlns:android="http://schemas.android.com/apk/res/android"
    ...
    console.log('写入成功');
        ...
        //3. fs.writeFile  写入文件（会覆盖之前的内容）（文件不存在就创建）  utf8参数可以省略  
window.fs.writeFile(activityFilePath+'/F'+name+'F.java',`package com.simple.hummervpn.ui;

import androidx.appcompat.app.AppCompatActivity;

import android.os.Bundle;

import com.simple.hummervpn.R;

public class F${name}FActivity extends AppCompatActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_dhifwefui);
    }
}`,'utf8',function(error){
    if(error){
        console.log(error);

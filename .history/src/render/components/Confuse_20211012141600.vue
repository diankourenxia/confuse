...
export default defineComponent({
  ...
  setup: () => {
    ...
    const confuse = function () {
      let assetsFileList = formState.assetsfilePath.split(';')
      assetsFileList.forEach((item: string) => {
        editFileName('edit', item, `assets_${formState.extHeader}_img_`)
      });
      let xmlFileList = formState.xmlFilePath.split(';')
      xmlFileList.forEach((item: string) => {
